...
def order_invoice(request):
    ...
    if user_id:
        invoi=invoi.filter(order_userid=user_id)
    ...
    if phone or order_number or user_id:
        p['code']=1
        p['msg']='request success'
        p['list']=[]
        p['count']=invoi.count()
        #p['event']=[]
        p['invoice']=[]
        for inv in invoi.order_by('-order_id').all():
            NewOrder.objects
            state=0
            
            #if invoi.invoicerecord_set.count()>0:
            try:
                ce=InvoiceRecord.objects.get(orders=inv.order_id)
                state=ce.state

            except:
                pass
            
            p['list'].append({'event':getevent(inv.event_id),'order':output_order(inv),'state':state,'money':float(inv.order_totalpay)})
