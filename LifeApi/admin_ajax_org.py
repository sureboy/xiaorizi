...
def Addr_data_str(request,query=None):
    ...
    if Addr:
        p = [ { "address":Add.address,"title":Add.title, "id":Add.id ,"city":Add.city.district_name if Add.city  else ''} for Add in Addr[:20] ]
...
def sponsor_data_str(request, query=None):
    ...
    if ms:
        p = [ { "name": s.name, "id": s.id, "is_verify": s.is_verify, 'event_count': s.event_count if s.event_count else 0} for s in ms[:20] ]
    response = json.dumps(p)
    return HttpResponse(response, mimetype="application/json")
...
def get_event_sponsor(request):
    ...
    if eid:
        ...
        result = [{ "name": s.name, "id": s.id, "is_verify": s.is_verify, 'event_count': s.event_count if s.event_count else 0} for s in ms]
    else:
        pass
...
def Addr_data(request,query):
    ...
    p=[ {"address":dis.address, "id":dis.id ,"title":dis.title,"city":dis.city.id if dis.city  else '',"city_name":dis.city.district_name if dis.city  else '' } for dis in diss ]
...
def get_event_cat(request):
    ...
    if eid:
        ...
        e_all_cat = e.cat.all()
        # 获得所有的分类，并筛选出需要的分类
        all_cat = cat_iter(exclude_id=exclude_id)
        need_cat = []
