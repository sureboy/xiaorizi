    ...
    (function (window) {
        ...
        $(".saveEvent").click(function () {
            ...
            for(var i=0;i<$("#source_sponsor_table_body tr").length;i++){
                ...
                addsponsorObj.pic=$("#source_sponsor_table_body tr").eq(i).find("td").eq(3).text();
        ...
        function address_map(){
            ...
            if(_this.parent().text().length!=0) {
                ...
                //map.centerAndZoom(discity, 15);  // 初始化地图,设置中心点坐标和地图级别
                
                
                map.getPoint("北京市海淀区上地10街", function(point){
            		if (point) {
        ...
        $(".addSponsorBtn").click(function(){
            ...
            if(sponsorName==""){
                ...
                for(var q=0;q<nameArr.length;q++){
                    if(nameArr[q]){
                        var c=nameArr[q].split("|")
                        if(c[1]==undefined) {
                          c[1]="";
                        }
                            $("#source_sponsor_table_body").append("<tr data-sponsor-name='"+c[0]+"'><td><span class='icon-remove deleteNewSponsor'></span>"+c[0]+"</td><td>false</td><td>0</td><td>"+c[1]+"<span class='glyphicon glyphicon-edit edit_sponsor'></span></td></tr>")
                    }
...
//价格管理
    ...
    $(".imgSearch").click(function () {
        window.open($(this).attr("href"),'newwindow','height=600,width=1000,top=0,left=0,toolbar=no,menubar=no,scrollbars=no,resizable=no,location=no,status=no')
        return false;
    })
});
