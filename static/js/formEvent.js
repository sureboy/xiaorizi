    ...
    (function (window) {
        ...
        $(".saveEvent").click(function () {
            ...
            for(var i=0;i<$("#source_address_table_body tr").length;i++){
                ...
                addressObj.venue_point=$("#source_address_table_body tr").eq(i).find("td").eq(3).text();
            ...
            $(".paraUl li").each(function (index) {
                ...
                if (name != '') {
                    if ($(this).hasClass('current')) {
                        var paraph = $($("#cke_editEvent").find("iframe").get(0).contentWindow.document.body).html();
                    } else {
                        if (id.indexOf('新增段落') != '-1') {
                            var paraph = $(".editEvent_addNew" + id.substring(4)).text();
                        } else {
                            var paraph = $(".editEvent_" + id).text();
                        }
                    }
                    var para = {};
                    if (id.indexOf('新增段落') == '-1') {
            ...
            } else if ($(".paraUl li").length == 0 && $($("#cke_editEvent").find("iframe").get(0).contentWindow.document.body).html() != '') {
                var para = {};
                para.tabname = '活动介绍';
                para.txt = $($("#cke_editEvent").find("iframe").get(0).contentWindow.document.body).html();
                para.order = 1;
                paraArray[paraArray.length] = JSON.stringify(para);
                paraJson.txtlist = paraArray;
                $.ajax({
        ...
        //    if (activeId.indexOf('addNewEvent') == -1) {
        //        var maxIndex = 0;
        ...
        //            }
        //        });
        //        var maxIndexForText = maxIndex + 1;
        //        $(parentTmp).find("#tabDiv .rightListFrame li.active a span.textSpan").text('新增数据' + maxIndexForText);
        //        $(parentTmp).find("#tabDiv .rightListFrame li.active a").attr('href', '#addNewEvent' + maxIndex);
        //        $(parentTmp).find("#tabDiv .tab-content .active").addClass('addNewEvent').attr('id', 'addNewEvent' + maxIndex);
        //    }
        //}
        //主办发搜索
        ...
        function address_map(){
            ...
            map.centerAndZoom(discity, 11);  // 初始化地图,设置中心点坐标和地图级别
            ...
            map.addEventListener("click", function(e){
                ...
                geoc.getLocation(pt, function(rs){
                    ...
                    _this.parent().html("lat:"+rs.point.lat+",lng:"+rs.point.lng)
                    $("#baidu_map").hide();
                });
            });
            $("#baidu_map").show();
        ...
        $(".addSponsorBtn").click(function(){
            ...
            if($(".addSponsor").val().length==0){
                $(".addSponsor").val($.trim(sponsorName))
            }else{
                $(".addSponsor").val($(".addSponsor").val()+"，"+$.trim(sponsorName))
            }
            $('#selectedSponsor').show();
            ...
            if(sponsorName==""){
                ...
                for(var q=0;q<nameArr.length;q++){
                    if(nameArr[q]){
                        $("#source_sponsor_table_body").append("<tr data-sponsor-name='"+nameArr[q]+"'><td><span class='icon-remove deleteNewSponsor'></span>"+nameArr[q]+"</td><td>false</td><td>0</td></tr>")
                    }
    ...
    function two(){
        ...
        var tag2_url="/newevent/get_json_cat/"+tag2_id+"/?_=1411541284292";
            if($(this).attr("data-click")=="true"){
                ...
                if($("#cat").val().length!=0){
                    $("#cat").val($("#cat").val()+","+$(this).find("a").attr("data-tag-id"));
                }else{
...
//价格管理
        ...
        $.get(
            ...
            ,function(data){
                ...
                $(".edit_price").bind("click",edit_price);
        ...
        function edit_price(){
            ...
            for(var i=0;i<price_tr.find("td").length;i++){
                ...
                if(i==1){
                    ...
                    price_tr.find("td").eq(7).html("<span class='glyphicon glyphicon-ok-circle price_gly' id='ok_gly'></span>"+"<span class='glyphicon glyphicon-remove-circle price_gly' id='no_gly'></span>");
                }else{
                    price_tr.find("td").eq(i).html("<input type='text' value='"+price_tr.find("td").eq(i).text()+"'>");
                }
            }
            $("#ok_gly").bind("click",ok_price);
            $("#no_gly").bind("click",no_price);
        }
    function ok_price(){
        var price_tr = $(this).parent().parent();
        for(var i=0;i<price_tr.find("td").length;i++){
            ...
            if(i==1){
                ...
                console.log(price_tr.find("td").eq(5).find("select[checked] ").text());
