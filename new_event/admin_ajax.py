...
from new_event.common import sendMail,find_cat_fid
from admin_self.common import oldEventToNewEvent,NewCatUrl,event_city_cat,NewformatEvent,ip_Filter
...
def Addr_data_str(request,query=None):
    ...
    #\s
    #re.sub(ur"[^\u4e00-\u9fa5\w]", " ", oldevent.event_search)
    #query=re.sub(ur"[\s]", "",  unicode(query))
    query=eval(repr(query).replace('\u200b',''))
    p=[]
